...
def opportunities_create_or_update(request):
    ...
    # Optional: minimum assessment score (0-100)
    min_score_raw = data.get("minAssessmentScore")
    min_assessment_score = None
    if min_score_raw is not None and str(min_score_raw).strip() != "":
        try:
            min_assessment_score = int(min_score_raw)
            if min_assessment_score < 0 or min_assessment_score > 100:
                return Response({"error": "minAssessmentScore must be between 0 and 100"}, status=400)
        except (TypeError, ValueError):
            return Response({"error": "minAssessmentScore must be an integer"}, status=400)
...
def apply_to_opportunity(request):
    ...
    applicant_email = _clean_text(data.get("email"))
    ...
    if not statement:
        return Response({"error": "statementOfInterest is required"}, status=400)
...
def faculty_applications(request):
    ...
    for app in apps:
        ...
        enriched.append({
            ...
            "statementOfInterest": app.statementOfInterest or "",
